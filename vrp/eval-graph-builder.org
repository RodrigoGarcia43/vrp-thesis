...
**** code
#+BEGIN_SRC lisp +n -r :results none :exports none :tangle ../src/eval-graph-builder.lisp
(defmacro def-var (name init-value &optional (graph *graph*))
...
**** code
#+BEGIN_SRC lisp +n -r :results none :exports none :tangle ../src/eval-graph-builder.lisp
(defmacro increment-distance (from-client to-client incremented-var distance-matrix &optional (graph *graph*))
...
**** code
#+BEGIN_SRC lisp +n -r :results none :exports none :tangle ../src/eval-graph-builder.lisp
(defmacro increment-value (target-var value &optional (graph *graph*))
...
**** code
#+BEGIN_SRC lisp +n -r :results none :exports none :tangle ../src/eval-graph-builder.lisp
  (defmacro return-value (var &optional (graph *graph*))
...
**** code
#+BEGIN_SRC lisp +n -r :results none :exports none :tangle ../src/eval-graph-builder.lisp
(defmacro decrement-demand (target-client demand-var &optional (graph *graph*))
...
**** code
#+BEGIN_SRC lisp +n -r :results none :exports none :tangle ../src/eval-graph-builder.lisp
  (defmacro apply-penalty (demand-var result-var penalty-factor &optional (graph *graph*))
...
#+BEGIN_SRC lisp +n -r :results output :exports both :tangle ../src/eval-graph-builder-test.lisp
  (with-basic-clients (1 2 3 4 5 6)
	(let* ((v1 (basic-vehicle 1))
	       ...
	       (s (basic-cvrp-solution :id 1 :routes (list r1 r2)))
	       (graph (init-graph s)))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
  (defun move-node-from-to (c1 c2 output-slot d-matrix &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod defallocable (output-slot value-of-slot &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
  (defmethod increment-with (output-slot partial-slot &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod set-output (output-slot &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod deliver (c capacity-slot-accumulator &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod penalize (input-slot-accum output-slot-accum p-factor &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod remove-client-from (route-index client-index &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod insert-client-to (buffer-index route-index client-index &optional (graph *graph*))
    (let* ((target-route (car (nthcdr route-index (routes (solution-track graph)))))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod get-value (var-symbol &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod get-output (&optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod set-route-vehicle (route-instance route-accumulator &optional (graph *graph*))
...
*** code
#+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
(defmethod remove-vehicle-from (route-index &optional (graph *graph*))
...
*** code
# #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
#   (defmethod insert-vehicle-to (route-index vehicle-index &optional (graph *graph*))
#     (let* ((target-route (car (nthcdr route-index (routes (solution-track graph)))))
 ...
 *** code
# #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
# (defmethod change-first-depot (route-index d-index &optional (graph *graph*))
#     (let* ((target-route (car (nthcdr route-index (routes (solution-track graph)))))
...
*** code
# #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
# (defmethod undo-first-depot-change (route-index &optional (graph *graph*))
#     (let* ((target-route (car (nthcdr route-index (routes (solution-track graph)))))
#            (reference-client (car (clients target-route)))
#            (io-reference (gethash reference-client (class-to-io graph)))
#            (depot-to-extract (first-depot target-route))
...
*** code
# #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
# (defmethod change-last-depot (route-index d-index &optional (graph *graph*))
#     (let* ((target-route (car (nthcdr route-index (routes (solution-track graph)))))
...
*** code
# #+BEGIN_SRC lisp +n -r :results none :exports code :tangle ../src/eval-graph-builder.lisp
# (defmethod undo-last-depot-change (route-index &optional (graph *graph*))
#     (let* ((target-route (car (nthcdr route-index (routes (solution-track graph)))))
#            (reference-client (car (last (clients target-route) 2)))
#            (io-reference (gethash reference-client (class-to-io graph)))
#            (depot-to-extract (car (last (clients target-route))))
