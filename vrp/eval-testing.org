...
#+BEGIN_SRC lisp +n -r :results none :exports both :tangle ../src/eval-tutorial.lisp
  ...
  (defparameter c2 (basic-cvrp-client 2 7))
  ...
  (defparameter v1 (cvrp-vehicle 1 9))
  (defparameter v2 (cvrp-vehicle 2 18))
...
;; In this case we use normal routes without depots
  (defparameter r3 (basic-route :id 1 :vehicle v1 :depot d0
					 :clients (list c1 c2 c3)))
  (defparameter r4 (basic-route :id 2 :vehicle v2 :depot d0
					 :clients (list c4 c5 c6)))

  (defparameter solution (basic-solution :id 1 :routes (list r3 r4)))
...
<EVAL-GRAPH: 
 ...
 client-buffer: #<HASH-TABLE :TEST EQL :COUNT 0 {100660BDA3}> 
...
#+begin_example
<EVAL-GRAPH: 
 output: <Accumulator, value: 50>  
 inputs: 
  <Depot node, content: <d:0>, demand: NIL>
  <Cliient node <c1: 1>, 
     first-distance-calculator: <output: <Accumulator, value: 7>>, 
     second-distance-calculator: <output: <Accumulator, value: 7>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: -3>>>
  <Cliient node <c2: 7>, 
     first-distance-calculator: <output: <Accumulator, value: 7>>, 
     second-distance-calculator: <output: <Accumulator, value: 7>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: -3>>>
  ...
  <Depot node, content: <d:0>, demand: NIL>
  <Cliient node <c4: 3>, 
     first-distance-calculator: <output: <Accumulator, value: 13>>, 
     second-distance-calculator: <output: <Accumulator, value: 13>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: 12>>>
  <Cliient node <c5: 2>, 
     first-distance-calculator: <output: <Accumulator, value: 13>>, 
     second-distance-calculator: <output: <Accumulator, value: 13>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: 12>>>
 ...
 client-buffer: #<HASH-TABLE :TEST EQL :COUNT 0 {1002E57E73}> 
...
Now let's extract client 2 from the solution. Client 2 is the client with index 2 in the route
with index 1 in our solution. We'll use =remove-client-from= method which takes as paramenters
the index of the route in the solution and the index of the client in the route and the graph.
All index are zero-based. When a client is removed from a solution it is appended to a list
in the tape which represents the previouslly removed clients.

#+BEGIN_SRC lisp +n -r :results none :exports both :tangle ../src/eval-tutorial.lisp
(do-operations (`(,#'remove-client-from 1 2 0 ,graph)))
#+END_SRC


The new value of the =total-distance= should be 19. Great upgrade made by removing the client with high demand.
...
<EVAL-GRAPH: 
 ...
 inputs: 
  <Depot node, content: <d:0>, demand: NIL>
  <Cliient node <c1: 1>, 
     first-distance-calculator: <output: <Accumulator, value: 6>>, 
     second-distance-calculator: <output: <Accumulator, value: 6>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: 4>>>
  <Cliient node <c2: 7>, 
     first-distance-calculator: <output: <Accumulator, value: 6>>, 
     second-distance-calculator: <output: <Accumulator, value: 6>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: 4>>>
  ...
  <Depot node, content: <d:0>, demand: NIL>
  <Vehicle node, content: <cv:1. 0/9>, accumulator: NIL>
  <Input node, content: <r1: <cv:1. 0/9> (<d:0>: (<c1: 1> <c3: 4> <d:0>))>>
  <Depot node, content: <d:0>, demand: NIL>
  <Cliient node <c4: 3>, 
     first-distance-calculator: <output: <Accumulator, value: 13>>, 
     second-distance-calculator: <output: <Accumulator, value: 13>>, 
 ...
 client-buffer: #<HASH-TABLE :TEST EQL :COUNT 1 {1003CAFE73}> 

 solution-track: S1: (0)
routes:
  <r1: <cv:1. 0/9> (<d:0>: (<c1: 1> <c3: 4> <d:0>))>
  <r2: <cv:2. 0/18> (<d:0>: (<c4: 3> <c5: 2> <c6: 1> <d:0>))>
>
#+end_example

... now let's insert the client extracted just after client 4 in the
second route.  We provide the =insert-client-to= with the index of the
client in the clients previouslly extracted list and the coordinates
of the client before we want to insert the new client to.

#+BEGIN_SRC lisp +n -r :results none :exports both :tangle ../src/eval-tutorial.lisp
(do-operations (`(,#'insert-client-to 2 2 0 ,graph)))
#+END_SRC
...
<EVAL-GRAPH: 
 ...
 inputs: 
  ...
  <Cliient node <c1: 1>, 
     ...
     demand-calculator: decrement-capacity: <Accumulator, value: 4>>>
  <Cliient node <c2: 7>, 
     first-distance-calculator: <output: <Accumulator, value: 15>>, 
     second-distance-calculator: <output: <Accumulator, value: 15>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: 5>>>
  <Cliient node <c3: 4>, 
     first-distance-calculator: <output: <Accumulator, value: 6>>, 
     second-distance-calculator: <output: <Accumulator, value: 6>>, 
     demand-calculator: decrement-capacity: <Accumulator, value: 4>>>
  ...
  <Depot node, content: <d:0>, demand: NIL>
  <Vehicle node, content: <cv:2. 0/18>, accumulator: NIL>
  <Input node, content: <r2: <cv:2. 0/18> (<d:0>: (<c4: 3> <c2: 7> <c5: 2>
                                                   <c6: 1> <d:0>))>>
 
 client-buffer: #<HASH-TABLE :TEST EQL :COUNT 0 {1003A5FE73}> 

 solution-track: S1: (0)
routes:
  <r1: <cv:1. 0/9> (<d:0>: (<c1: 1> <c3: 4> <d:0>))>
  <r2: <cv:2. 0/18> (<d:0>: (<c4: 3> <c2: 7> <c5: 2> <c6: 1> <d:0>))>
>
#+end_example
...
#+BEGIN_SRC lisp +n -r :results none :exports both :tangle ../src/eval-tutorial.lisp
  (do-suite-operations graph (list (op-select-client 1 2 0) 
...
: S1: (0)
: routes:
:   <r1: <cv:1. 0/9> (<d:0>: (<c1: 1>))>
:   <r2: <cv:2. 0/18> (<d:0>: (<c3: 4> <c4: 3> <c2: 7> <c6: 1> <d:0>))>
: 
...
<EVAL-GRAPH: 
 ...
 client-buffer: #<HASH-TABLE :TEST EQL :COUNT 0 {100515C3F3}> 
